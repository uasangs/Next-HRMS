...
import { 
  ...
  Users, Building, AlertCircle, User
} from 'lucide-react';

const UserAttendanceRequests = () => {
  // State management
  const [selectedRows, setSelectedRows] = useState(new Set());
  const [showFilter, setShowFilter] = useState(false);
  const [itemsPerPage, setItemsPerPage] = useState(20);
  ...
  // Mock data - filtered for current user (in real app, this would come from API)
  const currentUserId = 'FI-00001'; // This would come from auth context
  const currentUserName = 'Developer Daiyan'; // This would come from auth context
  ...
  const mockData = [
    {
      ...
      dateSubmitted: '2025-08-21',
      duration: '2 days',
      canCancel: true // Only submitted/pending requests can be cancelled
    },
    {
      id: 'HR-ARQ-25-07-009',
      employeeName: 'Developer Daiyan',
      ...
      dateSubmitted: '2025-08-20',
      duration: '3 days',
      canCancel: false // Approved requests typically can't be cancelled
    },
    {
      id: 'HR-ARQ-25-07-010',
      employeeName: 'Developer Daiyan',
    ...
    },
    {
      ...
      dateSubmitted: '2025-08-18',
      duration: '1 day',
      canCancel: false // Rejected requests can't be cancelled
    },
    {
      id: 'HR-ARQ-25-07-012',
      employeeName: 'Developer Daiyan',
  ...
  // Filter data to show only current user's requests
  const userRequests = mockData.filter(item => item.employeeId === currentUserId);

  // Get unique values for filters
  const uniqueRequestTypes = [...new Set(userRequests.map(item => item.requestType))].sort();
  ...
  const filteredData = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'dateSubmitted':
          ...
          break;
        case 'requestType':
          aVal = a.requestType.toLowerCase();
          bVal = b.requestType.toLowerCase();
          break;
        default:
          aVal = a[sortBy];
          bVal = b[sortBy];
    ...
    return result;
  }, [searchTerm, statusFilter, priorityFilter, requestTypeFilter, dateFilter, sortBy, sortOrder, userRequests]);
  ...
  const toggleSelectAll = () => {
    if (selectedRows.size === paginatedData.length && paginatedData.length > 0) {
      ...
      // Only select requests that can be cancelled
      const cancellableRequests = paginatedData.filter(item => item.canCancel).map(item => item.id);
      setSelectedRows(new Set(cancellableRequests));
    }
  };

  const handleBulkCancel = () => {
    console.log('Bulk Cancel:', Array.from(selectedRows));
    // Here you would typically call an API to cancel selected requests
    setSelectedRows(new Set()); // Clear selection after action
  };

  const handleCancelRequest = (requestId) => {
    console.log('Cancel Request:', requestId);
    // Here you would call an API to cancel the specific request
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-4">
          ...
          <div className="flex items-center justify-between p-3 border-b border-gray-200">
            <div className="flex items-center gap-4">
              <h1 className="text-lg font-semibold text-gray-900">Request History</h1>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-72 relative">
                ...
                <input
                  type="text"
                  placeholder="Search by request ID, reason, or type..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-8 pr-3 py-1.5 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent"
                  style={{ focusRingColor: '#ec4899' }}
              ...
              <Link 
                to="/dashboard/regularise"
                className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-1 no-underline"
              >
                <Plus className="w-3 h-3" />
                New Request
              </Link>
            ...
            <div className="px-3 py-2 bg-gray-50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <span className="font-medium">Filters</span>
                  {activeFiltersCount > 0 && (
                    <span className="px-1 py-0.5 bg-blue-600 text-white text-xs rounded-full font-medium">
                      {activeFiltersCount}
                    </span>
                  )}
                  {showFilter ? <ChevronUp className="w-2.5 h-2.5" /> : <ChevronDown className="w-2.5 h-2.5" />}
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-xs text-gray-600">
                  {selectedRows.size > 0 ? `${selectedRows.size} selected` : `${filteredData.length} requests`}
                </span>
                <Calendar className="w-3 h-3 text-gray-400" />
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-red-700 bg-red-100 border border-red-300 rounded hover:bg-red-200 transition-colors"
                  >
                    <XCircle className="w-3 h-3" />
              ...
              <thead className="bg-gray-50">
                <tr>
                  <th className="w-6 p-2">
                    <input
                      type="checkbox"
                      checked={selectedRows.size > 0 && selectedRows.size === paginatedData.filter(item => item.canCancel).length}
                      onChange={toggleSelectAll}
                  ...
                  <th className="text-left p-2 text-xs font-medium text-gray-600">Reason</th>
                  <th className="text-left p-2 text-xs font-medium text-gray-600">Duration</th>
                  <th className="w-20 p-2 text-center text-xs font-medium text-gray-600">
                    <Calendar className="w-3 h-3 mx-auto" />
                    <div className="text-xs mt-0.5">Date</div>
                  </th>
                  <th className="w-16 p-2 text-center text-xs font-medium text-gray-600">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {paginatedData.map((item) => (
                  <tr 
                    ...
                    style={{ 
                      borderLeftColor: selectedRows.has(item.id) ? '#2563eb' : 'transparent',
                      borderLeftWidth: selectedRows.has(item.id) ? '3px' : '0px'
                    }}
                  >
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      <div>
                        <span className="text-gray-900 text-xs font-mono">{item.id}</span>
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className={`px-1 py-0.5 rounded text-xs font-medium border ${getPriorityColor(item.priority)}`}>
                            {item.priority}
                          </span>
                        ...
                        {item.canCancel ? (
                          ...
                          <span className="text-xs text-gray-400">
                            {item.status === 'Approved' ? 'Approved' : 
                             item.status === 'Rejected' ? 'Rejected' : 'N/A'}
                          </span>
                        )}
                      </div>
                    </td>
            ...
            {paginatedData.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  {activeFiltersCount > 0 || searchTerm 
                    ? 'Try adjusting your search or filter criteria'
                    : 'You have no attendance requests yet'
                  }
                </p>
                <Link 
                  to="/dashboard/regularise"
                  className="mt-3 inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors no-underline"
                >
                  <Plus className="w-3 h-3" />
            ...
            <div className="flex items-center gap-1">
              ...
              {[20, 100, 500].map((size) => (
                <button
                  ...
                  className={`px-1.5 py-0.5 text-xs rounded transition-colors border ${
                    itemsPerPage === size
                      ? 'text-white font-medium border-blue-500 bg-blue-600'
                      : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100 border-gray-300'
                  }`}
                >
                  {size}
...
export default UserAttendanceRequests;