...
import "./MonthlyAttendance.css";
import { fetchAttendanceRecord } from "../Home/dashboardApi";
import dayjs from "dayjs";
...
const MonthlyAttendance = () => {
  ...
  useEffect(() => {
    const employeeId = localStorage.getItem("employee_id") || "123"; // fallback
    const fetchData = async () => {
      try {
        const records = await fetchAttendanceRecord(employeeId);
        console.log("API Records:", records);

        const formatted = records.map(r => {
          const parsed = dayjs(r.date);
          return {
            ...r,
            working_hours: r.duration || "--",
            month: parsed.isValid() ? parsed.format("MMMM") : "--",
            day: parsed.isValid() ? parsed.format("ddd") : "--",
            dateFormatted: parsed.isValid() ? parsed.format("DD MMM") : r.date,
            lc: "--",
        ...
        {monthRecords.length > 0 ? (
          monthRecords.map((row, i) => (
            <div className="Atd-table-row" key={i}>
              ...
              <span>{row.in_time || "--"}</span>
              <span>{row.out_time || "--"}</span>
              <span className={`Atd-status ${row.status?.toLowerCase()}`}>{row.status || "--"}</span>
              <span>{row.working_hours}</span>
              <span>{row.lc}</span>
              <span>{row.weekHrs}</span>
